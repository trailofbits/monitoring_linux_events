...
/*
  ...
  Once the program is running, use the following commands to emit new events:
...
std::unique_ptr<llvm::Module> generateExitBPFModule(llvm::LLVMContext &context,
  ...
  auto time_consumed = builder.CreateBinOp(llvm::Instruction::Sub,
...
int main() {
  ...
  if (sample_count == 0) {
    ...
    std::cout << "Average syscall run time: "
              << (total_time_used / sample_count) << " nsecs\n";
  }
