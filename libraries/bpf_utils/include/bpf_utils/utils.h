...
#include <bpf_utils/bpfmap.h>
#include <bpf_utils/sectionmemorymanager.h>
...
llvm::Value *bpfMapLookupElem(llvm::IRBuilder<> &builder, llvm::Value *key,
  ...
  auto callee = function;
#else
  auto callee = llvm::FunctionCallee(function_type, function);
#endif

  return builder.CreateCall(callee, {map_address, key});
}
...
llvm::Value *bpfPerfEventOutput(llvm::IRBuilder<> &builder, llvm::Value *ctx,
  ...
  return builder.CreateCall(
      callee, {ctx, map_address, builder.getInt64(flags), data, size});
}
...
bool readPerfEventArray(PerfBufferList &data,
  ...
  if (perf_buffer_context.empty()) {
    auto processor_count = getPossibleProcessorCount();
...
bool createPerfEventArray(PerfEventArray &obj, std::size_t page_count) {
  auto processor_count =
      static_cast<std::uint32_t>(getPossibleProcessorCount());
